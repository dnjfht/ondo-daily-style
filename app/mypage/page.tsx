...
import type { SavedLookSnapshot } from "@/lib/types";
...
const situationLabel: Record<SavedLookSnapshot["situation"], string> = { daily: "데일리", work: "출근", date: "데이트" };

type Profile = { personal_color: string | null; body_type: string | null; preferred_style: string | null; analysis_completed_at: string | null; gender: string | null; age_range: string | null };

function savedLookFromRow(value: unknown, savedAt: string): SavedLookSnapshot | null {
  ...
  return { ...look, savedAt } as SavedLookSnapshot;
}

export default async function MyPage() {
  let profile: Profile | null = null;
  let savedLooks: SavedLookSnapshot[] = [];
  let signedIn = false;
  ...
  if (hasSupabaseEnv()) {
    ...
    if (user) {
      ...
      const [{ data: profileData }, { data: savedLookRows }] = await Promise.all([
        supabase.from("profiles").select("personal_color,body_type,preferred_style,analysis_completed_at,gender,age_range").eq("id", user.id).maybeSingle(),
        supabase.from("saved_looks").select("look,saved_at").eq("user_id", user.id).order("saved_at", { ascending: false }),
      ]);
      profile = profileData;
      savedLooks = (savedLookRows ?? []).flatMap((row) => {
        const look = savedLookFromRow(row.look, row.saved_at);
        return look ? [look] : [];
      });
    }
  }
  ...
  return <main className="shell mypage">
    ...
    {!signedIn ? <section className="empty-state"><h2>로그인 후 나만의 결과를 확인하세요.</h2><p>간편 로그인으로 저장한 퍼스널컬러, 골격 유형, 취향, 룩을 이곳에서 다시 확인할 수 있어요.</p><Link className="primary" href="/login">로그인하기 ↗</Link></section> : <>
      ...
      {!profile || !profile.analysis_completed_at ? <section className="empty-state"><h2>아직 저장된 스타일 분석이 없어요.</h2><p>간단한 셀프 체크를 마치면 성별·연령대·날씨·상황을 함께 반영해 다음 추천을 조정합니다.</p><Link className="primary" href="/profile">스타일 분석 시작하기 ↗</Link></section> : <section className="my-style-card"><p className="eyebrow">SAVED STYLE PROFILE</p><h2>{colorLabel[profile.personal_color ?? ""] ?? "미설정"} 톤 · {bodyLabel[profile.body_type ?? ""] ?? "미설정"}</h2><div><span><b>무드</b>{styleLabel[profile.preferred_style ?? ""] ?? "미설정"}</span><span><b>성별</b>{genderLabel[profile.gender ?? ""] ?? "응답하지 않음"}</span><span><b>연령대</b>{ageLabel[profile.age_range ?? ""] ?? "응답하지 않음"}</span><span><b>저장일</b>{new Intl.DateTimeFormat("ko-KR", { dateStyle: "medium" }).format(new Date(profile.analysis_completed_at))}</span></div><Link className="primary" href="/profile">결과 수정하기 ↗</Link></section>}
      <section className="saved-looks-card"><p className="eyebrow">SAVED LOOKS</p><h2>저장한 룩</h2><p className="saved-looks-intro">저장 당시의 상황, 도시, 기온과 체감온도를 함께 기록합니다.</p>{savedLooks.length === 0 ? <p className="saved-looks-empty">아직 저장한 룩이 없어요. 마음에 드는 오늘의 코디를 저장해 보세요.</p> : <div className="saved-looks-grid">{savedLooks.map((look) => <article className="saved-look" key={`${look.id}-${look.savedAt}`}><div className="saved-look-image" style={{ backgroundImage: `url(${look.imageUrl})` }}><span>{situationLabel[look.situation]}</span></div><div><p className="saved-look-date">{new Intl.DateTimeFormat("ko-KR", { dateStyle: "medium" }).format(new Date(look.savedAt))}</p><h3>{look.title}</h3><p>{look.weather.city} · {look.weather.temperature}° · 체감 {look.weather.apparent}°</p><small>{look.reason}</small></div></article>)}</div>}</section>
    </>}
  </main>;
}
