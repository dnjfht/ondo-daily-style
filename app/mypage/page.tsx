...
type Profile = { personal_color: string | null; body_type: string | null; analysis_completed_at: string | null; gender: string | null; age_range: string | null; preferred_style: string | null; style_preferences: Record<string, string> | null };
  ...
  if (hasSupabaseEnv()) {
    ...
    if (user) {
      ...
      const [{ data: profileData }, { data: savedLookRows }] = await Promise.all([
        supabase.from("profiles").select("personal_color,body_type,analysis_completed_at,gender,age_range,preferred_style,style_preferences").eq("id", user.id).maybeSingle(),
